    ...
    if ($resNivel2 && $resNivel2->num_rows > 0) {
        while ($fila = $resNivel2->fetch_assoc()) {
            if ($fila['tipo'] === 'boton') {
                ...
                $url = trim($fila['url_destino'] ?? '');
        
                if (!empty($url)) {
                    // Si tiene una URL, usar como redirección
                    echo '
                    <div class="chatbox__body__message chatbox__body__message--left" style="display: flex; gap: 10px; margin-top: 10px;">
                        <a href="' . $url . '" class="btn botones" target="_blank" style="flex: 1;">' . $texto . '</a>
                    </div>';
                } else {
                    // Si no hay URL, llamar a cargarNivel3()
                    echo '
                    <div class="chatbox__body__message chatbox__body__message--left" style="display: flex; gap: 10px; margin-top: 10px;">
                        <a href="#' . $submenu . '" class="btn botones" onClick="cargarNivel3(\'' . $submenu . '\')" style="flex: 1;">' . $texto . '</a>
                    </div>';
                }
            }
        }
